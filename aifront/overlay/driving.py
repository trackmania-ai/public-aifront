# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/overlay_driving.ipynb.

# %% auto 0
__all__ = ['action_callback_args', 'progress_callback_args', 'action_overlay', 'progress_overlay']

# %% ../../nbs/overlay_driving.ipynb 1
import dash_bootstrap_components as dbc
import dash_svg as dsvg
from .default import background_color
from dash import Input, Output, State, html

# %% ../../nbs/overlay_driving.ipynb 3
...
# %% ../../nbs/overlay_driving.ipynb 4
def action_overlay(color):
    svg = dsvg.Svg(
    ...
    return dbc.Fade(
        dbc.Container(
            ...
            style=dict(width="5.6em", height="5.6em"),
            class_name="position-relative m-2",
        ),
        id="action-overlay",
        is_in=False,
    )
...
# %% ../../nbs/overlay_driving.ipynb 6
def bar_badge(children, class_name=""):
    class_name = f"position-absolute top-50 {class_name}"
    return dbc.Badge(
        children,
        pill=True,
        color="rgba(0,0,0,0)",
...
def progress_overlay():
    ...
    map_bar = dbc.Progress(
        ...
        striped=True,
        animated=True,
        style={"background-color": background_color, "height": "2em"},
    )
    map_badges = [
        bar_badge(
            [
    ...
    map_col = dbc.Col([map_bar, *map_badges], class_name="position-relative p-0 me-2")
    ...
    return dbc.Fade(
        dbc.Row([map_col, respawn_col], class_name="g-0"),
        id="progress-overlay",
        is_in=False,
    )
...
progress_callback_args = [
    ...
    function(
        ...
        var racetime = new Date(data.racetime).toISOString().slice(14, -1);
        var cpMax = data.trajectory.nb_max_checkpoints - 1;
        var cpCount = Math.min(data.checkpoints.length, cpMax);
        var respawnCount = data.trajectory.respawn_count;
        var mistakeDuration = data.trajectory.mistake_duration;
        var respawnMax = data.trajectory.max_respawn;
        if(data.must_respawn) {
            progress = data.trajectory.respawn_progress * 100;
            respawnCount++;
            mistakeDuration = 0;
        }
        var respawnBarCount = respawnCount;
        if(data.done && !data.finished) {
            respawnBarCount++;
            mistakeDuration = 0;
        }
        var respawnBarValue = respawnBarCount / (respawnMax + 1) * 100;
        var mistakeBarValue = mistakeDuration / data.trajectory.max_mistake_duration / (respawnMax + 1) * 100;
        return [
            progress,
            progress.toFixed(1),
            cpCount,
    ...
    """,
    Output("map-bar", "value"),
    Output("progress", "children"),
    Output("cp-count", "children"),
    Output("cp-max", "children"),
    Output("racetime", "children"),
    Output("respawn-bar", "value"),
    Output("mistake-bar", "value"),
    Output("respawn-count", "children"),
    Output("respawn-max", "children"),
    Output("action-overlay", "is_in"),
    Output("progress-overlay", "is_in"),
